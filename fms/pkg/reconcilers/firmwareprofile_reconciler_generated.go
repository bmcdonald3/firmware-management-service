...
// This file provides the generated boilerplate for FirmwareProfile reconciler.
//
...
//
// To customize reconciliation logic, edit firmwareprofile_reconciler.go
package reconcilers
...
// FirmwareProfileReconciler reconciles FirmwareProfile resources.
//
// This reconciler:
//   - Observes FirmwareProfile resources and updates their Status
...
//
// The implementation of reconcileFirmwareProfile() is in firmwareprofile_reconciler.go
type FirmwareProfileReconciler struct {
	reconcile.BaseReconciler

	// Custom fields are defined in firmwareprofile_reconciler.go
}

// NewDefaultFirmwareProfileReconciler creates a default FirmwareProfile reconciler.
//
...
//
// Returns:
//   - *FirmwareProfileReconciler: Initialized reconciler
func NewDefaultFirmwareProfileReconciler(client reconcile.ClientInterface, eventBus events.EventBus) *FirmwareProfileReconciler {
	return &FirmwareProfileReconciler{
...
func (r *FirmwareProfileReconciler) GetResourceKind() string {
	return "FirmwareProfile"
}

// Reconcile brings FirmwareProfile to desired state.
//
// This method is called:
//   - When a FirmwareProfile resource is created/updated/deleted
...
//   - resource: The FirmwareProfile resource to reconcile
//
// Returns:
//   - Result: Indicates if/when to requeue
//   - error: If reconciliation failed
func (r *FirmwareProfileReconciler) Reconcile(ctx context.Context, resource interface{}) (reconcile.Result, error) {
	...
	var res v1.FirmwareProfile // This is the typed struct
	...
	r.Logger.Debugf("Reconciling FirmwareProfile %s/%s", res.Kind, res.GetUID())

	// Call custom reconciliation logic (now passing &res)
	if err := r.reconcileFirmwareProfile(ctx, &res); err != nil {
		r.Logger.Errorf("Reconciliation failed for FirmwareProfile %s: %v", res.GetUID(), err)
	...
	if err := r.UpdateStatus(ctx, &res); err != nil {
		r.Logger.Errorf("Failed to update status for FirmwareProfile %s: %v", res.GetUID(), err)
	...
	/*
		...
		eventType := "io.openchami.inventory.firmwareprofiles.reconciled"
		if err := r.EmitEvent(ctx, &res, eventType); err != nil {
			r.Logger.Warnf("Failed to emit event for FirmwareProfile %s: %v", res.GetUID(), err)
