...
// This file provides the generated boilerplate for UpdateProfile reconciler.
//
...
//
// To customize reconciliation logic, edit updateprofile_reconciler.go
package reconcilers
...
// UpdateProfileReconciler reconciles UpdateProfile resources.
//
// This reconciler:
//   - Observes UpdateProfile resources and updates their Status
...
//
// The implementation of reconcileUpdateProfile() is in updateprofile_reconciler.go
type UpdateProfileReconciler struct {
	reconcile.BaseReconciler

	// Custom fields are defined in updateprofile_reconciler.go
}

// NewDefaultUpdateProfileReconciler creates a default UpdateProfile reconciler.
//
...
//
// Returns:
//   - *UpdateProfileReconciler: Initialized reconciler
func NewDefaultUpdateProfileReconciler(client reconcile.ClientInterface, eventBus events.EventBus) *UpdateProfileReconciler {
	return &UpdateProfileReconciler{
...
func (r *UpdateProfileReconciler) GetResourceKind() string {
	return "UpdateProfile"
}

// Reconcile brings UpdateProfile to desired state.
//
// This method is called:
//   - When a UpdateProfile resource is created/updated/deleted
...
//   - resource: The UpdateProfile resource to reconcile
//
// Returns:
//   - Result: Indicates if/when to requeue
//   - error: If reconciliation failed
func (r *UpdateProfileReconciler) Reconcile(ctx context.Context, resource interface{}) (reconcile.Result, error) {
	...
	var res v1.UpdateProfile // This is the typed struct
	...
	r.Logger.Debugf("Reconciling UpdateProfile %s/%s", res.Kind, res.GetUID())

	// Call custom reconciliation logic (now passing &res)
	if err := r.reconcileUpdateProfile(ctx, &res); err != nil {
		r.Logger.Errorf("Reconciliation failed for UpdateProfile %s: %v", res.GetUID(), err)
	...
	if err := r.UpdateStatus(ctx, &res); err != nil {
		r.Logger.Errorf("Failed to update status for UpdateProfile %s: %v", res.GetUID(), err)
	...
	/*
		...
		eventType := "io.openchami.inventory.updateprofiles.reconciled"
		if err := r.EmitEvent(ctx, &res, eventType); err != nil {
			r.Logger.Warnf("Failed to emit event for UpdateProfile %s: %v", res.GetUID(), err)
