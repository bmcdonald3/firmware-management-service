...
// This file provides the generated boilerplate for FirmwareManifest reconciler.
//
...
//
// To customize reconciliation logic, edit firmwaremanifest_reconciler.go
package reconcilers
...
// FirmwareManifestReconciler reconciles FirmwareManifest resources.
//
// This reconciler:
//   - Observes FirmwareManifest resources and updates their Status
...
//
// The implementation of reconcileFirmwareManifest() is in firmwaremanifest_reconciler.go
type FirmwareManifestReconciler struct {
	reconcile.BaseReconciler

	// Custom fields are defined in firmwaremanifest_reconciler.go
}

// NewDefaultFirmwareManifestReconciler creates a default FirmwareManifest reconciler.
//
...
//
// Returns:
//   - *FirmwareManifestReconciler: Initialized reconciler
func NewDefaultFirmwareManifestReconciler(client reconcile.ClientInterface, eventBus events.EventBus) *FirmwareManifestReconciler {
	return &FirmwareManifestReconciler{
...
func (r *FirmwareManifestReconciler) GetResourceKind() string {
	return "FirmwareManifest"
}

// Reconcile brings FirmwareManifest to desired state.
//
// This method is called:
//   - When a FirmwareManifest resource is created/updated/deleted
...
//   - resource: The FirmwareManifest resource to reconcile
//
// Returns:
//   - Result: Indicates if/when to requeue
//   - error: If reconciliation failed
func (r *FirmwareManifestReconciler) Reconcile(ctx context.Context, resource interface{}) (reconcile.Result, error) {
	...
	var res v1.FirmwareManifest // This is the typed struct
	...
	r.Logger.Debugf("Reconciling FirmwareManifest %s/%s", res.Kind, res.GetUID())

	// Call custom reconciliation logic (now passing &res)
	if err := r.reconcileFirmwareManifest(ctx, &res); err != nil {
		r.Logger.Errorf("Reconciliation failed for FirmwareManifest %s: %v", res.GetUID(), err)
	...
	if err := r.UpdateStatus(ctx, &res); err != nil {
		r.Logger.Errorf("Failed to update status for FirmwareManifest %s: %v", res.GetUID(), err)
	...
	/*
		...
		eventType := "io.openchami.inventory.firmwaremanifests.reconciled"
		if err := r.EmitEvent(ctx, &res, eventType); err != nil {
			r.Logger.Warnf("Failed to emit event for FirmwareManifest %s: %v", res.GetUID(), err)
