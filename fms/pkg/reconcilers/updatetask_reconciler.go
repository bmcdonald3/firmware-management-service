...
const (
	...
	firmwareInventory = "/redfish/v1/UpdateService/SoftwareInventory"
)
...
func (r *UpdateTaskReconciler) executePreflight(ctx context.Context, res *v1.UpdateTask, ip, user, pass string) error {
	...
	imageURI := fmt.Sprintf("http://%s/library/%s.bin", fmsHost, res.Spec.FirmwareRef)
	payload := fmt.Sprintf(`{"ImageURI":%q, "Targets":["/redfish/v1/UpdateService/SoftwareInventory/%s"]}`, imageURI, res.Spec.TargetComponent)

	var taskURI string
	var err error

	if dryRun {
		r.Logger.Infof("UpdateTask %s: Dry run enabled - simulating SimpleUpdate POST with payload: %s", res.GetUID(), payload)
		taskURI = "/redfish/v1/TaskService/Tasks/dry-run-123"
	} else {
...
func (r *UpdateTaskReconciler) monitorTask(ctx context.Context, res *v1.UpdateTask, ip, user, pass string) error {
	...
	if dryRun {
		r.Logger.Infof("UpdateTask %s: Dry run enabled - simulating task completion", res.GetUID())
		res.Status.State = "Success"
		return r.Client.Update(ctx, res)
	}
