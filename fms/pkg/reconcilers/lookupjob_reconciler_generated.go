...
// This file provides the generated boilerplate for LookupJob reconciler.
//
...
//
// To customize reconciliation logic, edit lookupjob_reconciler.go
package reconcilers
...
// LookupJobReconciler reconciles LookupJob resources.
//
// This reconciler:
//   - Observes LookupJob resources and updates their Status
...
//
// The implementation of reconcileLookupJob() is in lookupjob_reconciler.go
type LookupJobReconciler struct {
	reconcile.BaseReconciler

	// Custom fields are defined in lookupjob_reconciler.go
}

// NewDefaultLookupJobReconciler creates a default LookupJob reconciler.
//
...
//
// Returns:
//   - *LookupJobReconciler: Initialized reconciler
func NewDefaultLookupJobReconciler(client reconcile.ClientInterface, eventBus events.EventBus) *LookupJobReconciler {
	return &LookupJobReconciler{
...
func (r *LookupJobReconciler) GetResourceKind() string {
	return "LookupJob"
}

// Reconcile brings LookupJob to desired state.
//
// This method is called:
//   - When a LookupJob resource is created/updated/deleted
...
//   - resource: The LookupJob resource to reconcile
//
// Returns:
//   - Result: Indicates if/when to requeue
//   - error: If reconciliation failed
func (r *LookupJobReconciler) Reconcile(ctx context.Context, resource interface{}) (reconcile.Result, error) {
	...
	var res v1.LookupJob // This is the typed struct
	...
	r.Logger.Debugf("Reconciling LookupJob %s/%s", res.Kind, res.GetUID())

	// Call custom reconciliation logic (now passing &res)
	if err := r.reconcileLookupJob(ctx, &res); err != nil {
		r.Logger.Errorf("Reconciliation failed for LookupJob %s: %v", res.GetUID(), err)
	...
	if err := r.UpdateStatus(ctx, &res); err != nil {
		r.Logger.Errorf("Failed to update status for LookupJob %s: %v", res.GetUID(), err)
	...
	/*
		...
		eventType := "io.openchami.inventory.lookupjobs.reconciled"
		if err := r.EmitEvent(ctx, &res, eventType); err != nil {
			r.Logger.Warnf("Failed to emit event for LookupJob %s: %v", res.GetUID(), err)
