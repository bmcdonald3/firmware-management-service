...
// This file provides the generated boilerplate for UpdateJob reconciler.
//
...
//
// To customize reconciliation logic, edit updatejob_reconciler.go
package reconcilers
...
// UpdateJobReconciler reconciles UpdateJob resources.
//
// This reconciler:
//   - Observes UpdateJob resources and updates their Status
...
//
// The implementation of reconcileUpdateJob() is in updatejob_reconciler.go
type UpdateJobReconciler struct {
	reconcile.BaseReconciler

	// Custom fields are defined in updatejob_reconciler.go
}

// NewDefaultUpdateJobReconciler creates a default UpdateJob reconciler.
//
...
//
// Returns:
//   - *UpdateJobReconciler: Initialized reconciler
func NewDefaultUpdateJobReconciler(client reconcile.ClientInterface, eventBus events.EventBus) *UpdateJobReconciler {
	return &UpdateJobReconciler{
...
func (r *UpdateJobReconciler) GetResourceKind() string {
	return "UpdateJob"
}

// Reconcile brings UpdateJob to desired state.
//
// This method is called:
//   - When a UpdateJob resource is created/updated/deleted
...
//   - resource: The UpdateJob resource to reconcile
//
// Returns:
//   - Result: Indicates if/when to requeue
//   - error: If reconciliation failed
func (r *UpdateJobReconciler) Reconcile(ctx context.Context, resource interface{}) (reconcile.Result, error) {
	...
	var res v1.UpdateJob // This is the typed struct
	...
	r.Logger.Debugf("Reconciling UpdateJob %s/%s", res.Kind, res.GetUID())

	// Call custom reconciliation logic (now passing &res)
	if err := r.reconcileUpdateJob(ctx, &res); err != nil {
		r.Logger.Errorf("Reconciliation failed for UpdateJob %s: %v", res.GetUID(), err)
	...
	if err := r.UpdateStatus(ctx, &res); err != nil {
		r.Logger.Errorf("Failed to update status for UpdateJob %s: %v", res.GetUID(), err)
	...
	/*
		...
		eventType := "io.openchami.inventory.updatejobs.reconciled"
		if err := r.EmitEvent(ctx, &res, eventType); err != nil {
			r.Logger.Warnf("Failed to emit event for UpdateJob %s: %v", res.GetUID(), err)
