...
// This file provides the generated boilerplate for DeviceProfile reconciler.
//
...
//
// To customize reconciliation logic, edit deviceprofile_reconciler.go
package reconcilers
...
// DeviceProfileReconciler reconciles DeviceProfile resources.
//
// This reconciler:
//   - Observes DeviceProfile resources and updates their Status
...
//
// The implementation of reconcileDeviceProfile() is in deviceprofile_reconciler.go
type DeviceProfileReconciler struct {
	reconcile.BaseReconciler

	// Custom fields are defined in deviceprofile_reconciler.go
}

// NewDefaultDeviceProfileReconciler creates a default DeviceProfile reconciler.
//
...
//
// Returns:
//   - *DeviceProfileReconciler: Initialized reconciler
func NewDefaultDeviceProfileReconciler(client reconcile.ClientInterface, eventBus events.EventBus) *DeviceProfileReconciler {
	return &DeviceProfileReconciler{
...
func (r *DeviceProfileReconciler) GetResourceKind() string {
	return "DeviceProfile"
}

// Reconcile brings DeviceProfile to desired state.
//
// This method is called:
//   - When a DeviceProfile resource is created/updated/deleted
...
//   - resource: The DeviceProfile resource to reconcile
//
// Returns:
//   - Result: Indicates if/when to requeue
//   - error: If reconciliation failed
func (r *DeviceProfileReconciler) Reconcile(ctx context.Context, resource interface{}) (reconcile.Result, error) {
	...
	var res v1.DeviceProfile // This is the typed struct
	...
	r.Logger.Debugf("Reconciling DeviceProfile %s/%s", res.Kind, res.GetUID())

	// Call custom reconciliation logic (now passing &res)
	if err := r.reconcileDeviceProfile(ctx, &res); err != nil {
		r.Logger.Errorf("Reconciliation failed for DeviceProfile %s: %v", res.GetUID(), err)
	...
	if err := r.UpdateStatus(ctx, &res); err != nil {
		r.Logger.Errorf("Failed to update status for DeviceProfile %s: %v", res.GetUID(), err)
	...
	/*
		...
		eventType := "io.openchami.inventory.deviceprofiles.reconciled"
		if err := r.EmitEvent(ctx, &res, eventType); err != nil {
			r.Logger.Warnf("Failed to emit event for DeviceProfile %s: %v", res.GetUID(), err)
