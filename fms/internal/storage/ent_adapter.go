...
import (
	...
	"github.com/bmcdonald3/fms/internal/storage/ent/annotation"
...
func FromEntResource(ctx context.Context, entResource *ent.Resource) (interface{}, error) {
	...
	case "DeviceProfile":
		...
		if err := json.Unmarshal(entResource.Spec, &resource.Spec); err != nil {
			return nil, fmt.Errorf("failed to unmarshal spec for DeviceProfile: %w", err)
		}
		...
		if len(entResource.Status) > 0 && string(entResource.Status) != "null" {
			if err := json.Unmarshal(entResource.Status, &resource.Status); err != nil {
				return nil, fmt.Errorf("failed to unmarshal status for DeviceProfile: %w", err)
			}
		}
	...
	case "FirmwareProfile":
		...
		// Unmarshal Spec
		if err := json.Unmarshal(entResource.Spec, &resource.Spec); err != nil {
			return nil, fmt.Errorf("failed to unmarshal spec for FirmwareProfile: %w", err)
		}

		// Unmarshal Status
		if len(entResource.Status) > 0 && string(entResource.Status) != "null" {
			if err := json.Unmarshal(entResource.Status, &resource.Status); err != nil {
				return nil, fmt.Errorf("failed to unmarshal status for FirmwareProfile: %w", err)
			}
		}
	...
	case "LookupJob":
		...
		// Unmarshal Spec
		if err := json.Unmarshal(entResource.Spec, &resource.Spec); err != nil {
			return nil, fmt.Errorf("failed to unmarshal spec for LookupJob: %w", err)
		}

		// Unmarshal Status
		if len(entResource.Status) > 0 && string(entResource.Status) != "null" {
			if err := json.Unmarshal(entResource.Status, &resource.Status); err != nil {
				return nil, fmt.Errorf("failed to unmarshal status for LookupJob: %w", err)
			}
		}
	...
	case "UpdateJob":
		...
		// Unmarshal Spec
		if err := json.Unmarshal(entResource.Spec, &resource.Spec); err != nil {
			return nil, fmt.Errorf("failed to unmarshal spec for UpdateJob: %w", err)
		}

		// Unmarshal Status
		if len(entResource.Status) > 0 && string(entResource.Status) != "null" {
			if err := json.Unmarshal(entResource.Status, &resource.Status); err != nil {
				return nil, fmt.Errorf("failed to unmarshal status for UpdateJob: %w", err)
			}
		}
	...
	case "UpdateProfile":
		...
		// Unmarshal Spec
		if err := json.Unmarshal(entResource.Spec, &resource.Spec); err != nil {
			return nil, fmt.Errorf("failed to unmarshal spec for UpdateProfile: %w", err)
		}

		// Unmarshal Status
		if len(entResource.Status) > 0 && string(entResource.Status) != "null" {
			if err := json.Unmarshal(entResource.Status, &resource.Status); err != nil {
				return nil, fmt.Errorf("failed to unmarshal status for UpdateProfile: %w", err)
			}
		}
	...
	case "UpdateTask":
		...
		// Unmarshal Spec
		if err := json.Unmarshal(entResource.Spec, &resource.Spec); err != nil {
			return nil, fmt.Errorf("failed to unmarshal spec for UpdateTask: %w", err)
		}

		// Unmarshal Status
		if len(entResource.Status) > 0 && string(entResource.Status) != "null" {
			if err := json.Unmarshal(entResource.Status, &resource.Status); err != nil {
				return nil, fmt.Errorf("failed to unmarshal status for UpdateTask: %w", err)
			}
		}
