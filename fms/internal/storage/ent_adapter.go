...
import (
	...
	"github.com/bmcdonald3/firmware-management-service/fms/internal/storage/ent/annotation"
...
func FromEntResource(ctx context.Context, entResource *ent.Resource) (interface{}, error) {
	...
	case "FirmwareManifest":
		...
		// Unmarshal Spec
		if err := json.Unmarshal(entResource.Spec, &resource.Spec); err != nil {
			return nil, fmt.Errorf("failed to unmarshal spec for FirmwareManifest: %w", err)
		}

		// Unmarshal Status
		if len(entResource.Status) > 0 && string(entResource.Status) != "null" {
			if err := json.Unmarshal(entResource.Status, &resource.Status); err != nil {
				return nil, fmt.Errorf("failed to unmarshal status for FirmwareManifest: %w", err)
			}
		}
