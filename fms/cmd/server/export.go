...
Examples:
  ...
  fms export --kinds FirmwareManifest --output ./firmwaremanifest-backup
...
func runExport(ctx context.Context, format, output string, kinds []string, perType bool) error {
	...
	if len(kinds) > 0 {
		...
		resourceKinds = []string{"DeviceProfile", "FirmwareManifest", "LookupJob", "UpdateJob", "UpdateProfile"}
	}
...
func exportResourceKind(ctx context.Context, kind, output, format string, perType bool) (int, error) {
	...
	case "FirmwareManifest":
		items, e := storage.Queryfirmwaremanifests(ctx).All(ctx)
		if e != nil {
			return 0, fmt.Errorf("failed to query firmwaremanifests: %w", e)
		}
		for _, item := range items {
			resources = append(resources, item)
		}
